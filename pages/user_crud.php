  ...
  if (isset($_POST["insert"]) || isset($_POST["update"])) {
    ...
    if (isset($_POST["insert"])) {
      try {
        ...
        $sql = "INSERT INTO \"user\" (first_name, last_name, middle_name, birthdate, gender, type, email, password) VALUES (:fn, :ln, :mn, :bd, :gd, :tp, :em, :pw);";
        ...
        if ($statement->execute()) {
          if ($type === "student") {
            ...
            foreach ($subjects as $subject) {
              $sql = "INSERT INTO \"student_subject\" (subject_id, student_id) VALUES (:subject, :user)";
              $statement = $conn->prepare($sql);
            ...
            QRcode::png($lastId, $qrcode, 'H', 4, 4);

            $sql = "INSERT INTO \"student_code\" (student_id, qrcode) VALUES (:id, :code);";
            $statement = $conn->prepare($sql);
            $statement->bindParam(":id", $lastId, PDO::PARAM_INT);
...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
  ...
  if (isset($_GET["users"])) {
    $isForTeacher = isset($_GET["for_teacher"]) && $_GET["for_teacher"];
      ...
      while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($subject_statement->execute()) {
          ...
          while ($subject_row = $subject_statement->fetch(PDO::FETCH_ASSOC)) {
            $active = ($subject_row["stud_id"] == $user->id) ? true : false;
            $user->subjects[] = array(
  ...
  if (isset($_GET['search'])) {
    ...
    $sql = "SELECT * FROM \"user\" 
            WHERE id::text ILIKE :searchQuery 
            OR first_name ILIKE :searchQuery 
            OR last_name ILIKE :searchQuery 
            OR middle_name ILIKE :searchQuery 
            OR birthdate::text ILIKE :searchQuery 
            OR gender ILIKE :searchQuery 
            OR type ILIKE :searchQuery 
            OR email ILIKE :searchQuery 
            OR password ILIKE :searchQuery";
    ...
    if ($isForTeacher) {
      $sql = "SELECT DISTINCT u.* FROM \"user\" u
              ...
              AND (
                  u.id::text ILIKE :searchQuery 
                  OR u.first_name ILIKE :searchQuery 
                  OR u.last_name ILIKE :searchQuery 
                  OR u.middle_name ILIKE :searchQuery 
                  OR u.birthdate::text ILIKE :searchQuery 
                  OR u.gender ILIKE :searchQuery 
